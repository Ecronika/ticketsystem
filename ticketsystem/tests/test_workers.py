...
def test_create_worker(test_app):
    worker = WorkerService.create_worker("Test Worker", "1234", is_admin=False)
...
def test_duplicate_worker_error(test_app):
    WorkerService.create_worker("Dup", "1111")
    with pytest.raises(ValueError, match="existiert bereits"):
        WorkerService.create_worker("Dup", "2222")

def test_toggle_worker_status(test_app):
    worker = WorkerService.create_worker("ToggleMe", "5555")
    assert worker.is_active is True
...
def test_prevent_last_admin_deactivation(test_app):
    ...
    if not admin:
        admin = WorkerService.create_worker("Admin1", "0000", is_admin=True)
