...
def register_routes(bp):
    ...
    # csrf.exempt: login is protected by rate-limiting (5/min) + PIN; CSRF is
    # defence-in-depth here but session cookie issues in proxy setups can block
    # legitimate logins. Recovery PIN route is also exempted for the same reason.
    from extensions import csrf  # pylint: disable=import-outside-toplevel
    login_view = csrf.exempt(limiter.limit("5 per minute")(_login_view))
    ...
    recover_pin_view = csrf.exempt(limiter.limit("5 per minute")(_recover_pin_view))
