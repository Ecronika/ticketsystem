...
## [2.13.1] - 2026-03-16
### Changed
- UI: "Lehrjahr"-Badges auf Theme-System (`badge-subtle-info`) umgestellt für bessere Kontraste in Dark/HC Modes.
- UI: Kontrastverhältnis für Info-Badges im Dark Mode auf WCAG AA (>4.5:1) optimiert.
- Lokalisierung: Verbliebene englische "Add"-Buttons im JavaScript der Personalverwaltung übersetzt.

## [2.13.0] - 2026-03-16
### Added
- Feature: Prüfer können nun bearbeitet werden (inkl. Modal).
- UI: Kontrastverbesserung für das Lehrjahr-Badge (bg-primary-subtle).
- Lokalisierung: "Add"-Buttons in der Personalverwaltung auf "Hinzufügen" korrigiert.

## [2.12.42] - 2026-03-16

### Fixed
- **Routing**: Behebung eines 500er-Fehlers auf dem Dashboard durch Korrektur des Logout-Endpoints (`auth.logout` -> `main.logout`).
...
### Changed
- **Code Quality (Linter)**: Umstellung sämtlicher Logger-Aufrufe auf Lazy-Logging zur Vermeidung von Pylint `W1203`.
- **Refactoring (DTO)**: Vollständige Integration der `CheckSubmissionContext` Dataclass im Service-Layer zur Ablösung von ad-hoc Dictionaries.
- **Pylint**: Finaler Score von 10.00/10.00 ohne lokale Deaktivierungen (außer bewussten Architekturentscheidungen).
...
### Fixed
- **Hotfix (Python Scope)**: `UnboundLocalError` für `db` im Restore-Pfad behoben. Ein lokaler Import überschattete die globale Variable.
...
### Fixed
- **Migration-Idempotenz (Restore Fix)**: Die defensive Check-Logik wurde wieder in die Migrations-Scripts integriert. Dies ist zwingend erforderlich, um "Split-Brain"-Zustände (neues Schema vs. alte Migrations-Tabelle) abzufangen, die beim Restore von Datenbankdateien auftreten können.
...
### Fixed
- **Restore-Resilienz (Initialisierung)**: `app.py` prüft nun primär auf die Existenz der Kerntabelle `azubi` anstatt nur auf die Migrations-Historie. Dies stellt sicher, dass nach einem Restore alle Tabellen per `create_all` initialisiert werden, selbst wenn die `alembic_version` bereits vorhanden ist.

## [2.12.32] - 2026-03-15

### Fixed
- **Migration Robustness (Restore Fix)**: Die Migrationen prüfen nun vor der Spalten-Inspektion, ob die Tabellen überhaupt existieren. Dies verhindert `NoSuchTableError` bei komplexen Wiederherstellungsszenarien (Restore), in denen die `alembic_version` bereits existiert, aber die Tabellen noch fehlen.
...
### Fixed
- **Migration Idempotency**: Migration `3cefbb0dbee3` wurde so umgebaut, dass sie vor dem Hinzufügen von Spalten prüft, ob diese bereits existieren. Dies verhindert Start-Fehler in Docker-Umgebungen, in denen die Datenbank bereits manuell oder durch frühere Versionen gepatcht wurde.
